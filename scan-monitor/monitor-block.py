...
class Exercises:
    ...
    def split_msg_eth(self,dict):
        ...
        coin_adress=""
        # 传输的方法的文本
        if dict['method'] == "0x791ac947":
            a_input=dict['input'][-128:-64]
            coin_adress=a_input.replace(a_input[0:24],"0x")
        if dict['method'] == "0xb6f9de95":
            b_input=dict['input'][-64:]
            coin_adress = b_input.replace(b_input[0:24],"0x")
        ...
        return "发送方:{send}\n\n to:{recive}\n\n 合约: {coin_name}\n\n 方法：{method} \n\n 价值：{value} ETH。\n\n [debank]({link}) \n [区块transfer]({transfer}) \t [区块hash]({hash_link}) \t [opensea]({open_link}) \n\n 时间：{time}".\
            format(method=dict['method'],coin_name =coin_adress, transfer=transfer_link,hash_link=bscscan_hash_link,link=debank_link,open_link=opensea_link,value=float(dict['value']) / self.wei,time=self.stampTransformTime(int(dict['time'])), send=dict['from'],recive=dict['to'])
    ...
    def get_recent_tx(self,address,rotate_count=0):
        ...
        for item in apikey_list:
            ...
            if res == 443 and rotate_count < 10:  # 网络问题并且20次都访问都是443则报错停止运行
                ...
                # eth_list = funcdata.get_block_list(item)  #置为空列表调试用
                # eth_list=[]
                ...
                if first_mes['blockNumber'] not in funcdata.get_block_list(item) and time.time() - float(first_mes['timeStamp'])  < 60:
                    method = first_mes['input'][0:10]
                    self.dingding_warn({"time":first_mes['timeStamp'],"hash":first_mes['hash'],"value":first_mes['value'],"from":first_mes['from'],"to":first_mes['to'],'method':method,"input":first_mes['input']},item)
